...
-- #1697: refreshing the statement timestamp inside PL/iSQL must not leak
-- into the enclosing statement.  The nested call sleeps and reads sysdate
-- itself (its own statement timestamps advance), but afterwards the
-- enclosing statement's timestamp must be restored, so a subsequent read of
-- sysdate in the same statement still matches the first one.
CREATE FUNCTION sysdate_nested_fn() RETURNS text
LANGUAGE plisql AS $$
DECLARE d text;
BEGIN
  PERFORM pg_sleep(1);
...
END $$;
WITH
     before (a) AS MATERIALIZED
       (SELECT to_char(sysdate, 'YYYYMMDDHH24MISS')),
     nested (b) AS MATERIALIZED
       (SELECT sysdate_nested_fn() AS b FROM before),
     after (c) AS MATERIALIZED
       (SELECT to_char(sysdate, 'YYYYMMDDHH24MISS') FROM nested)
SELECT (a = c) AS outer_statement_restored,
       (b > a) AS nested_advanced
FROM before, nested, after;

-- #1697: a finished PL/iSQL call must not leave its statement timestamp
-- behind, or later statements in the same session would keep reading the
-- old value (SYSDATE/CURRENT_DATE would be frozen).  Read sysdate, run a
-- nested PL/iSQL call (which sleeps), then read sysdate again in a later
-- statement: the value must still advance.
CREATE TEMP TABLE sysdate_leak_test(a text, c text);
INSERT INTO sysdate_leak_test (a)
SELECT to_char(sysdate, 'YYYYMMDDHH24MISS');
SELECT count(*) AS nested_called FROM (SELECT sysdate_nested_fn()) s;
INSERT INTO sysdate_leak_test (c)
SELECT to_char(sysdate, 'YYYYMMDDHH24MISS');
SELECT (min(a) < min(c)) AS sysdate_advanced_after_nested
