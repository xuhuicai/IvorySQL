...
/*
 *	GetOracleStatementStartTimestampInternal
 *
 * Return the raw internal value (0 when PL/iSQL is not active), without
 * falling back to stmtStartTimestamp.  This is what PL/iSQL needs to save
 * and restore: the fallback must not be written back into
 * oraStmtStartTimestamp, or it would linger and freeze SYSDATE/CURRENT_DATE
 * for every later statement in the session.
 */
TimestampTz
GetOracleStatementStartTimestampInternal(void)
{
	return oraStmtStartTimestamp;
}
