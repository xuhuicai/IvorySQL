 ...
 * plisql_inline_handler().  DO blocks still use the simple_econtext_stack,
 ...
 * EState will be cleaned up when we get back to plisql_inline_handler.  This
 ...
 * LOOP_RC_PROCESSING will do nothing except reset "rc" to PLISQL_RC_OK.
 *
					 ...
					 bool atomic)
{
	 ...
	 */
	plerrcontext.callback = plisql_exec_error_callback;
...
/*
 * Helper for plisql_exec_function: coerce composite result to the specified
...
/* ----------
 * plisql_exec_trigger		Called by the call handler for
 *				trigger execution.
 * ----------
 */
HeapTuple
					...
					TriggerData *trigdata)
{
	 ...
	 * Setup the execution state
	 */
	plisql_estate_setup(&estate, func, NULL, NULL, NULL);
	...
	rec_new = (PLiSQL_rec *) (estate.datums[func->new_varno]);
	rec_old = (PLiSQL_rec *) (estate.datums[func->old_varno]);
...
/* ----------
 * plisql_exec_event_trigger		Called by the call handler for
 ...
 */
static void
plisql_exec_error_callback(void *arg)
{
				   ...
				   PLiSQL_function * func)
{
	...
	for (i = 0; i < ndatums; i++)
	{
		PLiSQL_datum *indatum = indatums[i];
		PLiSQL_datum *outdatum;

		/* This must agree with plisql_finish_datums on what is copiable */
		switch (indatum->dtype)
		{
			...
			case PLISQL_DTYPE_REC:
				outdatum = (PLiSQL_datum *) ws_next;
				memcpy(outdatum, indatum, sizeof(PLiSQL_rec));
				ws_next += MAXALIGN(sizeof(PLiSQL_rec));
				break;
					   ...
					   PLiSQL_var * var)
{
	...
	switch (var->promise)
	{
		...
		case PLISQL_PROMISE_TG_TABLE_NAME:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
		...
		case PLISQL_PROMISE_TG_TABLE_SCHEMA:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
...
exec_toplevel_block(PLiSQL_execstate * estate, PLiSQL_stmt_block * block)
{
	 ...
	 */
	save_timestamp = GetOracleStatementStartTimestampInternal();
...
exec_stmt_block(PLiSQL_execstate * estate, PLiSQL_stmt_block * block)
{
	...
	for (i = 0; i < block->n_initvars; i++)
	{
		...
		PLiSQL_datum *datum = estate->datums[n];

		/*
		 * The set of dtypes handled here must match plisql_add_initdatums().
		 *
		 ...
		 * PLiSQL_datum to PLiSQL_variable.
		 */
		estate->err_var = (PLiSQL_variable *) datum;
	...
	if (block->exceptions)
	{
		...
		PG_CATCH();
		{
			...
			{
				PLiSQL_exception *exception = (PLiSQL_exception *) lfirst(e);
				...
				if (exception_matches_conditions(edata, exception->conditions))
				{
					 ...
					 */
					PLiSQL_var *state_var;
					PLiSQL_var *errm_var;

					state_var = (PLiSQL_var *)
	...
	{
		case PLISQL_RC_OK:
		case PLISQL_RC_RETURN:
		case PLISQL_RC_CONTINUE:
			return rc;

		case PLISQL_RC_EXIT:
			if (estate->exitlabel == NULL)
				return PLISQL_RC_EXIT;
			if (block->label == NULL)
				return PLISQL_RC_EXIT;
...
exec_stmts(PLiSQL_execstate * estate, List *stmts)
{
	...
	foreach(s, stmts)
	{
		...
		/* Let the plugin know that we are about to execute this statement */
		if (*plisql_plugin_ptr && (*plisql_plugin_ptr)->stmt_beg)
			((*plisql_plugin_ptr)->stmt_beg) (estate, stmt);
		...
		/* Let the plugin know that we have finished executing this statement */
		if (*plisql_plugin_ptr && (*plisql_plugin_ptr)->stmt_end)
			((*plisql_plugin_ptr)->stmt_end) (estate, stmt);
...
exec_stmt_call(PLiSQL_execstate * estate, PLiSQL_stmt_call * stmt)
{
	...
	/*
	 * Also construct a DTYPE_ROW datum representing the plisql variables
...
/*
 * We construct a DTYPE_ROW datum representing the plisql variables
 * associated with the procedure's output arguments.  Then we can use
 * exec_move_row() to do the assignments.
 */
...
make_callstmt_target(PLiSQL_execstate * estate, PLiSQL_expr * expr)
{
	 ...
	 * an unadorned plisql variable (Datum), which we can insert into the row
	 * Datum.
	 */
	nfields = 0;
	for (i = 0; i < numargs; i++)
...
exec_stmt_if(PLiSQL_execstate * estate, PLiSQL_stmt_if * stmt)
{
	...
	{
		PLiSQL_if_elsif *elif = (PLiSQL_if_elsif *) lfirst(lc);
...
exec_stmt_fors(PLiSQL_execstate * estate, PLiSQL_stmt_fors * stmt)
{
	 ...
	 * Execute the loop
	 */
	rc = exec_for_query(estate, (PLiSQL_stmt_forq *) stmt, portal, true);
...
exec_stmt_forc(PLiSQL_execstate * estate, PLiSQL_stmt_forc * stmt)
{
	...
	if (stmt->argquery != NULL)
	{
		...
		set_args.cmd_type = PLISQL_STMT_EXECSQL;
	 ...
	 */
	rc = exec_for_query(estate, (PLiSQL_stmt_forq *) stmt, portal, false);
			...
			case PLISQL_DTYPE_VAR:
				{
					...
					/*
					 * A PLiSQL_var could not be of composite type, so
					 ...
					 * is NULL.  (Note that plisql_exec_trigger assumes that
		...
		for (cp = stmt->message; *cp; cp++)
		{
			...
			if (cp[0] == '%')
			{
				...
				paramvalue = exec_eval_expr(estate,
											(PLiSQL_expr *) lfirst(current_param),
											&paramisnull,
											&paramtypeid,
											&paramtypmod);
				...
				if (err_code)
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("RAISE option already specified: %s",
									"ERRCODE")));
				err_code = plisql_recognize_err_condition(extval, true);
					...
					ResourceOwner simple_eval_resowner)
{
	...
	/* this link will be restored at exit from plisql_call_handler */
	...
	estate->datums = NULL;
	/* the datums array will be filled by copy_plisql_datums() */
	...
	estate->paramLI->outparamSetup = (OutParamSetupHook) plisql_out_param_setup;
	...
	if (simple_eval_estate)
	{
		...
		ctl.keysize = sizeof(plisql_CastHashKey);
		ctl.entrysize = sizeof(plisql_CastHashEntry);
		ctl.hcxt = CurrentMemoryContext;
		estate->cast_hash = hash_create("PLiSQL private cast cache",
										16, /* start small and extend */
										&ctl,
	...
	else
	{
		...
		if (shared_cast_hash == NULL)
		{
			ctl.keysize = sizeof(plisql_CastHashKey);
			ctl.entrysize = sizeof(plisql_CastHashEntry);
			shared_cast_hash = hash_create("PLiSQL cast cache",
										   16,	/* start small and extend */
										   &ctl,
										   HASH_ELEM | HASH_BLOBS);
		}
	 ...
	 * PL/iSQL variables.  Note that we also give the plugin a few function
	 * pointers, so it can call back into PL/iSQL for doing things like
	 * variable assignments and stack traces.
	 */
	if (*plisql_plugin_ptr)
	{
		(*plisql_plugin_ptr)->error_callback = plisql_exec_error_callback;
		(*plisql_plugin_ptr)->assign_expr = exec_assign_expr;
		(*plisql_plugin_ptr)->assign_value = exec_assign_value;
		(*plisql_plugin_ptr)->eval_datum = exec_eval_datum;
		(*plisql_plugin_ptr)->cast_value = exec_cast_value;

		if ((*plisql_plugin_ptr)->func_setup)
			((*plisql_plugin_ptr)->func_setup) (estate, func);
	}
}
				  ...
				  PLiSQL_stmt_execsql * stmt)
{
	...
	long		tcount;
	int			rc;
	PLiSQL_expr *expr = stmt->sqlstmt;
	int			too_many_rows_level = 0;

	if (plisql_extra_errors & PLISQL_XCHECK_TOOMANYROWS)
		too_many_rows_level = ERROR;
	else if (plisql_extra_warnings & PLISQL_XCHECK_TOOMANYROWS)
	...
	switch (rc)
	{
		...
		case SPI_ERROR_COPY:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("cannot COPY to/from client in PL/iSQL")));
			break;

		case SPI_ERROR_TRANSACTION:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("unsupported transaction command in PL/iSQL")));
			break;
		...
		case PLISQL_DTYPE_ROW:
			{
				...
				if (isNull)
				{
					...
					exec_move_row(estate, (PLiSQL_variable *) row,
								  NULL, NULL);
				}
				else
				{
					...
					exec_move_row_from_datum(estate, (PLiSQL_variable *) row,
											 value);
				}
				break;
			}
		...
		case PLISQL_DTYPE_REC:
			{
				...
				else
				{
					...
					exec_move_row_from_datum(estate, (PLiSQL_variable *) rec,
											 value);
				}
				break;
			}
		...
		case PLISQL_DTYPE_RECFIELD:
			{
				 ...
				 */
				PLiSQL_recfield *recfield = (PLiSQL_recfield *) target;
				PLiSQL_rec *rec;
				ExpandedRecordHeader *erh;
...
/*
 * exec_eval_datum				Get current value of a PLiSQL_datum
 *
 * The type oid, typmod, value in Datum format, and null flag are returned.
 *
 * At present this doesn't handle PLiSQL_expr datums; that's not needed
...
/*
 * plisql_exec_get_datum_type				Get datatype of a PLiSQL_datum
 *
...
/*
 * plisql_exec_get_datum_type_info			Get datatype etc of a PLiSQL_datum
 *
 * An extended version of plisql_exec_get_datum_type, which also retrieves the
			   ...
			   Portal portal, bool prefetch_ok)
{
	...
	while (n > 0)
	{
		...
		for (i = 0; i < n; i++)
		{
			 ...
			 */
			if (var->dtype == PLISQL_DTYPE_REC)
			{
				PLiSQL_rec *rec = (PLiSQL_rec *) var;
 ...
 * PLISQL_exprs in this estate.  It contains no per-param data, just hook
 * functions, so it's effectively read-only for SPI.
 *
 * An exception from pure read-only-ness is that the parserSetupArg points
 * to the specific PLiSQL_expr being evaluated.  This is not an issue for
 ...
 * create a ParamListInfo struct for each PLiSQL_expr, but for the moment
...
/*
 * plisql_param_fetch		paramFetch callback for dynamic parameter fetch
 *
				   ...
				   ParamExternData *prm)
{
	...
	expr = (PLiSQL_expr *) params->parserSetupArg;
	...
	if (params->outparamSetup)
	{
		prm->pflags = 0;
	}
	else
	{
		/* always mark params as "const" for executor's purposes */
...
/*
 * plisql_param_compile		paramCompile callback for plisql parameters
 */
static void
					 ...
					 Datum *resv, bool *resnull)
{
	...
	else if (datum->dtype == PLISQL_DTYPE_RECFIELD)
...
/*
 * plisql_param_eval_var		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
...
/*
 * plisql_param_eval_var_ro		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
...
/*
 * plisql_param_eval_recfield		evaluation of EEOP_PARAM_CALLBACK step
 *
...
/*
 * plisql_param_eval_generic		evaluation of EEOP_PARAM_CALLBACK step
 *
...
/*
 * plisql_param_eval_generic_ro	evaluation of EEOP_PARAM_CALLBACK step
 *
			  ...
			  HeapTuple tup, TupleDesc tupdesc)
{
	 ...
	 */
	if (target->dtype == PLISQL_DTYPE_REC)
	{
		PLiSQL_rec *rec = (PLiSQL_rec *) target;
						  ...
						  TupleDesc tupdesc)
{
	...
	if (tupdesc != NULL)
	{
		if (plisql_extra_errors & PLISQL_XCHECK_STRICTMULTIASSIGNMENT)
			strict_multiassignment_level = ERROR;
		else if (plisql_extra_warnings & PLISQL_XCHECK_STRICTMULTIASSIGNMENT)
	...
	if (target->dtype == PLISQL_DTYPE_REC)
	{
		 ...
		 * types.  That's sort of a pain, but historically plisql has allowed
	...
	if (target->dtype == PLISQL_DTYPE_ROW)
	{
		...
		for (fnum = 0; fnum < row->nfields; fnum++)
		{
			...
			var = (PLiSQL_var *) (estate->datums[row->varnos[fnum]]);
			...
			exec_assign_value(estate, (PLiSQL_datum *) var,
							  value, isnull, valtype, valtypmod);
		}
...
do_cast_value(PLiSQL_execstate * estate,
			  ...
			  Oid reqtype, int32 reqtypmod)
{
	plisql_CastHashEntry *cast_entry;
 ...
 *
 * Returns a plisql_CastHashEntry if an expression has to be evaluated,
...
get_cast_hashentry(PLiSQL_execstate * estate,
				   ...
				   Oid dsttype, int32 dsttypmod)
{
	plisql_CastHashKey cast_key;
	plisql_CastHashEntry *cast_entry;
	plisql_CastExprHashEntry *expr_entry;
	bool		found;
	 ...
	 * hash table with the new tree because all plisql functions within a
...
/*
 * plisql_create_econtext --- create an eval_econtext for the current function
 *
...
/*
 * plisql_destroy_econtext --- destroy function's econtext
 *
...
/*
 * plisql_xact_cb --- post-transaction-commit-or-abort cleanup
 *
 * If a simple-expression EState was created in the current transaction,
 * it has to be cleaned up.  The same for the simple-expression resowner.
 */
void
plisql_xact_cb(XactEvent event, void *arg)
{
	/*
...
/*
 * plisql_subxact_cb --- post-subtransaction-commit-or-abort cleanup
 *
...
exec_eval_using_params(PLiSQL_execstate * estate, List *params)
{
	...
	foreach(lc, params)
	{
		PLiSQL_expr *param = (PLiSQL_expr *) lfirst(lc);
				   ...
				   const PLiSQL_expr * expr)
{
	...
	while ((dno = bms_next_member(expr->paramnos, dno)) >= 0)
	{
		...
		curvar = (PLiSQL_var *) estate->datums[dno];

		exec_eval_datum(estate, (PLiSQL_datum *) curvar,
...
/*
 * call plisql anonymous using out parameters.
 * this function would be used when jdbc calls plisql anonymous block.
